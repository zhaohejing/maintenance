(function () {
    appModule.controller('common.views.event.deal.index', [
        ...
        function ($scope, $uibModal,appSession, $stateParams, eventService, $state) {
            ...
            vm.sovled = function (fault) {
                eventService.updateFaultInfo({
                    id: fault.id, faultType: 3
                }).success(function () {
                    abp.notify.info("修理成功");
                    vm.jTableInfo.isShow = false;
                    vm.getFaults();
                }).finally(function () {
                });
            }
            vm.accept = function (fault) {
                eventService.updateFaultInfo({
                    id: fault.id, faultType: 2
                }).success(function () {
                    abp.notify.info("接受报修成功");
                    vm.jTableInfo.isShow = false;
                    vm.getFaults();
                }).finally(function () {
                });
