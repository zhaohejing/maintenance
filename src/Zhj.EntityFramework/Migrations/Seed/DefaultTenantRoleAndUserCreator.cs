...
namespace MyCompanyName.AbpZeroTemplate.Migrations.Seed
{
    ...
    public class DefaultTenantRoleAndUserCreator
    {
        ...
        public DefaultTenantRoleAndUserCreator(ZhjDbContext context)
        {
            _context = context;
        }
        ...
        private void CreateDefaultTenantAndUsers()
        {
            ...
            {
                adminUserForDefaultTenant = User.CreateTenantAdminUser(defaultTenant.Id,"管理员", "admin@defaulttenant.com", "123qwe");
            ...
            #region 创建工程师
            var grole = _context.Roles.FirstOrDefault(r => r.TenantId == null && r.Name == "工程师");
            if (grole == null) {
                grole = _context.Roles.Add(new Role(null, "工程师", "工程师") { IsStatic = true, IsDefault = false,TenantId= defaultTenant.Id });
                _context.SaveChanges();
            }
            var guser = _context.Users.FirstOrDefault(u => u.TenantId == null && u.UserName == "工程师");
            if (guser == null) {
                guser = _context.Users.Add(
                    new User {
                ...
                //Assign Admin role to admin user
                _context.UserRoles.Add(new UserRole(guser.Id, grole.Id));
                _context.SaveChanges();
                ...
                foreach (var permission in permissions) {
                    if (!permission.IsGrantedByDefault) {
                        if (!permission.Name.Contains("Pages.Administration")) {
                            _context.Permissions.Add(
                        new RolePermissionSetting {
                            Name = permission.Name,
                            IsGranted = true,
            ...
            #endregion
            #region 创建游客
            var nrole = _context.Roles.FirstOrDefault(r => r.TenantId == null && r.Name == "游客");
            if (nrole == null) {
                nrole = _context.Roles.Add(new Role(null, "游客", "游客") { IsStatic = true, IsDefault = true,TenantId=defaultTenant.Id });
                _context.SaveChanges();
            }
            var nuser = _context.Users.FirstOrDefault(u => u.TenantId == null && u.UserName == "游客");
            if (nuser == null) {
                nuser = _context.Users.Add(
                    new User {
                        TenantId = defaultTenant.Id,
                        UserName = "游客",
                        Name = "游客",
                ...
                //Assign Admin role to admin user
                _context.UserRoles.Add(new UserRole(nuser.Id, nrole.Id));
                _context.SaveChanges();
                ...
                foreach (var permission in permissions) {
                    if (!permission.IsGrantedByDefault) {
                        if (permission.Name.Contains("Pages.FaultRelease")) {
                            _context.Permissions.Add(
                           new RolePermissionSetting {
                               Name = permission.Name,
                               IsGranted = true,
                        ...
                        if (permission.Name.Contains("Pages.KnowledgeBase")) {
                            _context.Permissions.Add(
                           new RolePermissionSetting {
                               Name = permission.Name,
                               IsGranted = true,
