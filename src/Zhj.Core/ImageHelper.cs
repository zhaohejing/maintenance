...
namespace MyCompanyName.AbpZeroTemplate {
    public static class ImageHelper {
        ...
        #endregion
        #region 读取base64文件流 保存
        /// <summary>
        /// 读取base64文件流 保存
        /// </summary>
        /// <param name="strImage"></param>
        /// <param name="imgNewPathAll"></param>
        public static void SaveImage(string strImage, string bigpath, string smallpath) {
            //图片二进制
            byte[] imgByte = Convert.FromBase64String(strImage);
            //数据流
            System.IO.MemoryStream ms = new System.IO.MemoryStream(imgByte);
            using (Bitmap bitmap = new Bitmap(ms)) {
                //大图直接保存
                bitmap.Save(bigpath, System.Drawing.Imaging.ImageFormat.Png);
                Bitmap newbitmap;
                //大图 裁剪为正方形  压缩成小图 保存
                newbitmap = CutForSquare(bitmap, smallpath, bitmap.Width, 100);
                newbitmap = ResizeImage(newbitmap, 200, 200);
                newbitmap.Save(smallpath);
                newbitmap.Dispose();
            }
        }

        public static void FileSaving(byte[] file, string bigpath, string smallpath) {
            try {
                var dir = Path.GetDirectoryName(bigpath);
                ...
                //大图直接保存
                bitmap.Save(bigpath, System.Drawing.Imaging.ImageFormat.Jpeg);
        ...
        public static void FileSavingApp(byte[] file, string path) {
            try {
        ...
        public static void FileSavingApp(string file, string path, string name) {
            try {
                var dir = Path.GetDirectoryName(path);
                if (!Directory.Exists(dir)) {
                    Directory.CreateDirectory(dir);
        ...
        public static Bitmap CutForSquare(Bitmap bitmap, string fileSaveUrl, int side, int quality) {
            //创建目录
            string dir = Path.GetDirectoryName(fileSaveUrl);
            if (!Directory.Exists(dir))
                Directory.CreateDirectory(dir);
            ...
            if (bitmap.Width < side && bitmap.Height < side) {
                ...
                if (initWidth != initHeight) {
                    ...
                    if (initWidth > initHeight) {
                        ...
                        Rectangle toR = new Rectangle(0, 0, initHeight, initHeight);
                        //画图
                        pickedG.DrawImage(bitmap, toR, fromR, System.Drawing.GraphicsUnit.Pixel);
                        //重置宽
                        initWidth = initHeight;
                    }
                    //高大于宽的竖图
                    ...
                    //将截图对象赋给原图
                    bitmap = (System.Drawing.Bitmap)pickedImage.Clone();
                    return bitmap;
                }
                ...
                resultG.Clear(Color.White);
                //绘制缩略图
                resultG.DrawImage(bitmap, new System.Drawing.Rectangle(0, 0, side, side), new System.Drawing.Rectangle(0, 0, initWidth, initHeight), System.Drawing.GraphicsUnit.Pixel);
        ...
        #region 压缩图片到指定目录
        public static void SaveSmallImage(byte[] imgByte, string smallPath, string bigPath, string smallName, string bigName) {
            try {
                var dir = Path.GetDirectoryName(smallPath);
                if (!Directory.Exists(dir)) {
                    Directory.CreateDirectory(dir);
                }
                var dirBig = Path.GetDirectoryName(bigPath);
                if (!Directory.Exists(dirBig)) {
                    Directory.CreateDirectory(dirBig);
                }
                System.IO.MemoryStream ms = new System.IO.MemoryStream(imgByte);
                string smallpath = smallPath + "/" + smallName + ".jpg";
                string bigpath = bigPath + "/" + bigName + ".jpg";
                using (Bitmap bitmap = new Bitmap(ms)) {
                    //大图直接保存
                    bitmap.Save(bigpath, System.Drawing.Imaging.ImageFormat.Jpeg);
                    Bitmap newbitmap;
